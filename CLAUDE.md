...
- Une seule divergence assumée d'avec l'amont : `ui/input/input.svelte` pose `autocapitalize="sentences"` par défaut. Le navigateur ne lit cette consigne que sur le champ lui-même ou sur le `<form>` qui le possède, jamais sur un ancêtre, donc elle ne peut pas vivre ailleurs sans être répétée à chaque appel. Après un `npx shadcn-svelte@latest add input`, réapplique-la
...
- Laisse le cache HTTP du navigateur gérer les requêtes conditionnelles : dès qu'une réponse porte un `ETag` et `Cache-Control: no-cache`, il envoie `If-None-Match` de lui-même, reçoit le `304` et rend le corps qu'il avait gardé avec un statut `200` — n'écris ni mémorisation d'empreinte ni traitement du `304`, le code ne le voit jamais. Ça ne tient que si l'appel ne passe pas `cache: 'no-store'` à `fetch` et n'ajoute aucun paramètre anti-cache à l'URL : `send()` dans `src/lib/api.ts` s'en abstient, ne l'y réintroduis pas
- Dans `nginx/toutpris.conf`, seul `_app/immutable/` — les fichiers hachés par SvelteKit — part avec un cache d'un an. Tout le reste, `index.html` compris, se revalide : un fichier de `static/` est servi sous son nom tel quel et ne peut pas être mis en cache long
